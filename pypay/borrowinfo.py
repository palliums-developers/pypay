...
class BorrowInfo (QObject):
    def __init__(self, dict = None, parent = None):
        ...
        if dict is not None:
            ...
            self._pledge_rate = dict['pledge_rate'] # 质押率
    ...
    # pledge_rate
    pledge_rateChanged = pyqtSignal()

    @pyqtProperty(int, notify=pledge_rateChanged)
    def pledge_rate(self):
        return self._pledge_rate

    @pledge_rate.setter
    def pledge_rate(self, pledge_rate):
        self._pledge_rate = pledge_rate
        self.pledge_rateChanged.emit()
